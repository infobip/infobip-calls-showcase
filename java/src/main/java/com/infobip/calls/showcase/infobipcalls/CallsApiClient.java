...
import com.infobip.calls.showcase.infobipcalls.model.api.*;
import com.infobip.calls.showcase.infobipcalls.model.client.CallRequest;
import com.infobip.calls.showcase.infobipcalls.model.client.CallResponse;
import com.infobip.calls.showcase.infobipcalls.model.client.ConnectRequest;
import com.infobip.calls.showcase.infobipcalls.model.client.ConnectResponse;
...
import static com.infobip.calls.showcase.infobipcalls.model.api.ActionStatus.FAILED;
...
public class CallsApiClient {
    ...
    public CallsApiClient(@Value("https://${infobip.api-host}") String infobipBaseUrl,
                          ...
                          @Value("${infobip.calls-configuration-id}") String infobipCallsConfigurationId) {
        this.baseUrl = infobipBaseUrl;
        this.callsConfigurationId = infobipCallsConfigurationId;
    ...
    public CallResponse callPhone(CallRequest callRequest) {
        var createCallRequest = new CreateCallRequest(callsConfigurationId, new Endpoint(callRequest.getPhoneNumber()), callRequest.getCallerId());
    ...
    public void hangupCall(String callId) {
        var httpRequest = new HttpEntity<>(Map.of(), httpHeaders);
        restTemplate.postForEntity("%s/calls/1/calls/%s/hangup".formatted(baseUrl, callId), httpRequest, Void.class);
    }

    public void hangupConference(String conferenceId) {
        var httpRequest = new HttpEntity<>(Map.of(), httpHeaders);
        restTemplate.postForEntity("%s/calls/1/conferences/%s/hangup".formatted(baseUrl, conferenceId), httpRequest, Void.class);
    }
}
